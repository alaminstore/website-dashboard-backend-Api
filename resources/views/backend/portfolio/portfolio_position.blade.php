            ...
            <table id="myTable" class="table table-bordered dt-responsive nowrap"
                ...
                @foreach($portfolio_position as $position)
                    <tr class="text-center">
                        ...
                        <td>
                            <button type="button" class="btn btn-sm btn-outline-primary waves-effect waves-light category-edit" data-id="{{$position->portfolio_position_id}}" title="Edit" data-toggle="modal" data-target="#myModal">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ url('portfolioposition.updated') }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group row">
                                <label for="name" class="col-sm-2 col-form-label">Portfolio Category Name</label>
                                <div class="col-sm-10">
                                    <select style="width: 200px" id="nameid2" name="nameid">
                                        <option ></option>
                                        @foreach($portfolio_cat as $cat)
                                          <option value="{{$cat->portfolio_category_id}}">{{$cat->name}}</option>
                                        @endforeach
                            ...
                            <div class="form-group row">
                                <label for="name" class="col-sm-2 col-form-label">Portfolio Item</label>
                                <div class="col-sm-10">
                                    <select style="width: 200px" id="itemid2" name="itemid">
                                        <option></option>
                                        @foreach($portfolio_item as $item)
                                          <option value="{{$item->portfolio_item_id}}">{{$item->title}}</option>
                                        @endforeach
                            ...
                            <div class="form-group row">
                                <label for="portfolio_cat_icon" class="col-sm-2 col-form-label">Position</label>
                                <div class="col-sm-10">
                                    <select style="width: 200px" id="position2" name="position">
                                        <option></option>
                                          <option value="1">Position 1</option>
                                          <option value="2">Position 2</option>
                                          <option value="3">Position 3</option>
...
@section('scripts')
                ...
                $.ajax({
                    ...
                    success:function(data){
                      let url = window.location.origin;
                        ...
                        $('#category-edit-form').find('#nameid2').val(data.portfolio_category_id).focus();
                        $('#category-edit-form').find('#itemid2').val(data.portfolio_position_id;
                        $('#category-edit-form').find('#position2').val(data.position);
                        if(data.icon)
                          {
    ...
    <script type="text/javascript">
        ...
        $("#nameid,#nameid2").select2({
              placeholder: "Select a Portfolio Category Name",
            //   allowClear: true
          });
          $("#itemid,#itemid2").select2({
              placeholder: "Select a Portfolio Item",
            //   allowClear: true
          });
          $("#position,#position2").select2({
