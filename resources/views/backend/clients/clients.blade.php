                ...
                <div class="card-body">
                    ...
                    <form action="{{ url('clients/store') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-12">
                                <input class="form-control" type="text" id="name" name="name" placeholder="Client Name Here..." required>
                            </div>
                        </div>
            ...
            <table id="myTable" class="table table-bordered dt-responsive nowrap"
                ...
                @foreach($clients  as $client)
                    <tr class="text-center">
                        ...
                        <td>{{$client->name}}</td>
                        <td class="cat_img"><img src="{{$client->image}}" class="img-fluid" alt="portfolio Category Image">
                        </td>
                        <td>
                        ...
                        <td>
                            <button type="button" class="btn btn-sm btn-outline-primary waves-effect waves-light category-edit" data-id="{{$client->client_id}}" title="Edit" data-toggle="modal" data-target="#myModal">
                                <i class="mdi mdi-border-color"></i> Edit
                            </button>
                           <button class="btn btn-outline-danger btn-sm category-delete"  title="Delete" onclick="deleteData({{$client->client_id}})"><i class="ti-trash"></i> Delete</button>
                           <form id="delete-form-{{$client->client_id}}" method="post" action="{{route('clients.destroy', $client->client_id)}}" style="display: none">
                               @csrf
                               @method('DELETE')
                           </form>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="category-edit-form" action="{{ route('clients.updated') }}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="form-group row">
                                <label for="name" class="col-sm-2 col-form-label">Name</label>
                                <div class="col-sm-12">
                                    <input class="form-control" type="text" id="category-edit-name" name="name" placeholder="Client Name Here..." required>
                                </div>
                            </div>
                ...
                $.ajax({
                    ...
                    success:function(data){
                      let url = window.location.origin;
                        ...
                        var position =$('#category-edit-form').find('#position2').val(data.precedence);
